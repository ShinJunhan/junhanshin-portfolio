...
- **🏗️ Full IaC** — 41 AWS resources via Terraform + 8 Ansible roles, deployed with a single `make apply` command (~10 min)
- **📊 Integrated Monitoring** — Prometheus + Grafana + AlertManager (5s scrape interval, 5s alert-for duration)
...
- **💰 Cost Optimization** — NAT Instance used instead of NAT Gateway (lower cost, better response time)
...
| Hybrid connectivity | — | Tailscale VPN (proj-mgmt ↔ AWS) |
...
> ⭐ **The `pkill -x` pattern:** `pkill -f` also matches the Ansible shell command line's own text, which can terminate the recovery command itself — a self-termination trap. `pkill -x` matches only the exact process name.
...
```
main ──────────●────────────●────────────  (Hwijeong Cho, protected branch — for release/presentation)
               ▲            ▲
               │ PR         │ PR
               │            │
...
- `dev`: integration/testing branch (Junhan Shin, PR Owner — reviews, merges, and syncs)
