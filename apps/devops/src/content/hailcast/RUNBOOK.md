...
Four repositories, cloned as siblings inside one basket. This is not a
preference — `make -C ../project3-hailcast-infra` and the Docker credential
...
No Tailscale and no Ansible — this is a single-account, EKS, GitOps
architecture.
...
| `make infra-fmt` | no guard — it needs no credentials at all |
...
- **Set `AWS_PROFILE` in your shell, never in `.env`.** `.env` is read for one
  ...
  and Terraform end up looking at different accounts — `make infra-destroy`
...
flag is off — IRSA declares ten roles in code but only two exist if
`enable_app_irsa` is false.

What it checks:

- `karpenter.sh/discovery` on the private subnets and the **node** security
  group, and that the **value** is right — not merely that the two match
...
- `sqs:PurgeQueue` is nowhere — including where a wildcard would pull it in.
...
- missing `description` on security groups and IAM policies — these cannot be
  added later without recreating the resource
...
EventBridge Scheduler calls the AWS API directly — no Lambda — to stop the
system node group and RDS overnight. On by default
...
Three repositories deploy; ops does not — it is local tooling that creates no
AWS resource and no deployment definition.
...
| `manifests` | ArgoCD is watching. It syncs on its own — `selfHeal` and `prune` are on |
...
price** — the CUR integration was scoped and never finished, so treat every
figure it reports as an estimate.
...
| 4 | Who installs ArgoCD on rebuild | a named person + the install manifest | — |
...
load balancer deletion is asynchronous. If they are still listed, wait another
...
stage so the destroy is real; it never injects `FORCE=yes` — if an ALB or a
Karpenter node is still alive, a person has to see the warning and decide.
...
> "there is no ALB" and "I could not find out" are not the same answer.
...
runtime on its own. The node group took 8m10s, CloudFront 3m14s — against an
...
interrupt it** — that desynchronises state from reality.
...
edited by a person** — infrastructure produces them, the deployment track
applies them.
...
5. **app** — rebuild all six images with `workflow_dispatch`, **not** a `dev`
   push. A push builds only the changed services and the rest stay on
   `ImagePullBackOff`
...
9. **infra** — migrate the CUR bucket and recreate the definition. Not urgent;
   it does not collide with anything above
...
keys, not the sync status — you want all three of `DB_HOST`, `DB_PASSWORD` and
`DB_USER` present.
...
- **Cost allocation tags** — `Project`, `Environment`, `ManagedBy`. Activated
